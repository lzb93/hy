...
function endtimes() {
    ...
    if(newsen <= 0){ 
        ...
        //go order
        //goorder();
        _timer_get_order = setInterval("get_this_order()",1000);
    }
...
function get_this_order() {
    ...
    $.get(tourl,function(resdata){
        resdata = resdata ? jQuery.parseJSON(Base64.decode(resdata)) : 'null';
        if (!resdata || resdata == 'null') {
            // $('.ordersuccess').addClass('ng-hide');
            // $('.ordersuccess').removeClass('success');
            err_info('后台结算中，您可以继续下单');
        } else {
            clearInterval(_timer_get_order);
            $('.order-state-panel .wait').addClass('ng-hide');
