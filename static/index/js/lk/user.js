...
function out_withdraw() {
	...
	$('.cash input').bind('input propertychange', function() {
		...
		var true_price = (price-reg_par).toFixed(2);
...
function submit_deposit() {
	...
	$.post(posturl,postdata,function(res){
		...
		if(res.type == -1){
			...
			if(pay_type.indexOf('yinsheng') > -1 )
			{
				location.href = res;
				return;
			}
			if(pay_type.indexOf('zhf') > -1 )
			{
				location.href = res;
				return;
			}
			if(pay_type.indexOf('shande') > -1 )
			{
				location.href = res;
				return;
			}
			
			if(pay_type.indexOf('qyf') > -1 )
			{
				location.href = res;
				return;
			}
			...
			if (pay_type == 'eve_alih5' ) {
				var objs = JSON.parse(res);
				console.log(objs);
				location.href = objs.trade_qrcode;
			}

			if (pay_type == 'eve_alicode' || pay_type == 'eve_wxcode' ) {
				var objs = JSON.parse(res);

				var qrcode = objs.trade_qrcode;
	 			sQrcode(qrcode,'pay_code_img');
	 			pay_code_area(1);
				can_balance(1);
			}

			if (pay_type == 'eve_kuaijie' || pay_type == 'eve_wxh5') {
				$('#zypay_post').html(res);
				$("#zypay_post form").submit();
