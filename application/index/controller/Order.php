...
class Order extends Base {
    ...
    public function addorder() {
        ...
        $userMoney = $user['usermoney']; // 临时使用
        //订单号
        $adddata['orderno'] = date('YmdHis') . rand(1111, 9999);
        // 开启事物
        Db::name('order')->startTrans();
        //下单
        ...
        if ($ids) {
            ...
            $logData = [
                'order_no' => $adddata['orderno'],
                'order_money' => $allfee,
                'user_money' => $userMoney,
                'user_paid_money' => $userMoney - $allfee,
                'log_type' => '建仓',
            ...
            if ($editmoney) {
                ...
                $redisResult = $redis->zAdd('order_list', $adddata['selltime'], json_encode($adddata));
                if (!$redisResult) {
                    Db::name('order')->rollback(); // 回滚
                    return WPreturn('下单失败，请重试！', -1);
                }
                Db::name('order')->commit(); // 提交事物
