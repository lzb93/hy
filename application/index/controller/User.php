...
/*
				...
				if(!$can_cash){
					return WPreturn('代理余额不足，请联系客服',-1);
				}*/
...
*/
   		...
   		$data['btime'] = $nowtime;
   		$data['reg_par'] = 0;
   		$orderid = $data['balance_sn'] = 'HY'.$uid.'U'.$nowtime.rand(111111,999999);
   		$bankcode = $post['pay_type'];
   		$data['bpbalance'] = $user['usermoney'];
      	$yuming = $_SERVER["HTTP_HOST"];   
      
      //充值显示充值通道名称
		$wh['pay_conf'] = "name:".$post['pay_type'];          
		$wh['isdelete'] = "0";        
		$wh['is_use'] = "1";
        $payment = db('payment')->where($wh)->order('pay_order desc ')->find();  
 		$data['pay_type'] = $payment['pay_name'];
   		...
   		$data['bpid'] = $ids;
		// elt支付
		if (strpos($post['pay_type'], 'eltpay_') !== false) {
			$r = explode( "_", $post['pay_type'] );
			$st = $r[1];
			$url = "/index/elt_pay/recharge?orderid=".$orderid.'&st='.$st;
			return $url;
		}

		// tf支付
		if (strpos($post['pay_type'], 'tfpay_') !== false) {
			$r = explode( "_", $post['pay_type'] );
			$st = $r[1];
			$url = "/index/tf_pay/recharge?orderid=".$orderid.'&st='.$st;
			return $url;
		}

        // 合盛源支付
        if (strpos($post['pay_type'], 'hsy_') !== false) {
            $r = explode( "_", $post['pay_type'] );
            $st = $r[1];
            $url = "/index/hsy_pay/recharge?orderid=".$orderid.'&st='.$st;
            return $url;
        }

		// 坤泽支付
		if (strpos($post['pay_type'], 'kzpay_') !== false) {
			$r = explode( "_", $post['pay_type'] );
			$st = $r[1];
			$url = "/index/kz_pay/recharge_$st?orderid=".$orderid;
			return $url;
		}
		// 易智慧支付
		if (strpos($post['pay_type'], 'yzh_') !== false) {
			$r = explode( "_", $post['pay_type'] );
			$st = $r[1];
			$url = "/index/yzh_pay/recharge?money=".$money."&orderid=".$orderid.'&st='.$st;
			return $url;
		}
		// espay支付
		if (strpos($post['pay_type'], 'espay_') !== false) {
			$r = explode( "_", $post['pay_type'] );
			$st = $r[1];
			$url = "/index/es_pay/recharge?money=".$money."&orderid=".$orderid.'&st='.$st;
			return $url;
		}
		//掌灵
		if( strpos($post['pay_type'], 'zhangling' ) !== false  )
		{
			$r = explode( "_", $post['pay_type'] );
			$channelId = count($r) > 1 ? $r[1] : '';
			$url = "/index/zlyd_pay/recharge?money=".$money."&orderid=".$orderid.'&st='.$channelId;
			return $url;
		}

		//中南
		if( strpos($post['pay_type'], 'zhongnan_' ) !== false  )
		{
			$r = explode( "_", $post['pay_type'] );
			$bankcode = $r[1];
			$url = "/zhongnanpay/pay.php?money=".$money."&orderid=".$orderid.'&bankid='.$bankcode;
			return $url;
		}
		...
		//蜂鸟、海马
   		if($post['pay_type'] == 'fnfwxgzh'||$post['pay_type'] == 'fnfzfbsm'||$post['pay_type'] == 'fnfwxsm')
		{
			if($post['pay_type'] == 'fnfwxgzh'){
              
       			$url =  'http://ghgj.bkvhe.top/fengniaofu/payment/weixin_mppay.php?money='.$money.'&orderid='.$orderid.'&bankid='.$bankcode.'&yuming='.$yuming;
				return $url;
            }
			
			if($post['pay_type'] == 'fnfwxsm'){
              
       			$url =  '/fengniaofu/payment/weixin_qrcode.php?money='.$money.'&orderid='.$orderid.'&bankid='.$bankcode;
				return $url;
            }
			
			if($post['pay_type'] == 'fnfzfbsm'){
              
       			$url =  '/fengniaofu/payment/alipay_qrcode.php?money='.$money.'&orderid='.$orderid.'&bankid='.$bankcode;
				return $url;
            }
		}
		if( strpos( $post['pay_type'] , 'yinsheng') !== false  )
		{
			$url = '/yinsheng/pay.php?orderid='.$data['balance_sn'].'&money='.$data['bpprice'].'&bank='.$post['pay_type'];
   			return $url;
   		}		
		if($post['pay_type'] == 'yinfubaoyinlian'){
              
       			$url =  '/yinfubaopay/pay.php?money='.$money.'&orderid='.$orderid.'&bankid='.$bankcode;
				return $url;
            }
		if( strpos( $post['pay_type'] , 'qyf') !== false  )
		{
			$url = '/qyf/pay.php?orderid='.$data['balance_sn'].'&money='.$data['bpprice'].'&bank='.$post['pay_type'];
   			return $url;
   		}
      //易达方
        if( strpos($post['pay_type'], 'edf_' ) !== false  )
		{
          	$r = explode( "_", $post['pay_type'] );
            $bankcode = $r[1];
			$url = "/edfpay/pay.php?money=".$money."&orderid=".$orderid.'&bankid='.$bankcode;          
			return $url;
		}
	
      //智能云
        if( strpos($post['pay_type'], 'zny_' ) !== false  )
		{
          	$r = explode( "_", $post['pay_type'] );
            $bankcode = $r[1];
			$url = "/zhinengyunpay/pay.php?money=".$money."&orderid=".$orderid.'&bankid='.$bankcode;          
			return $url;
		}	   	
      ...
      //聚合
        if( strpos($post['pay_type'], 'juhe_' ) !== false  )
		{
          	$r = explode( "_", $post['pay_type'] );
            $bankcode = $r[1];
			$url = "/juhepay/pay.php?money=".$money."&orderid=".$orderid.'&bankid='.$bankcode.'&userid='.$uid;          
			return $url;
		}
	
	
      //聚合2
        if( strpos($post['pay_type'], 'juhe2_' ) !== false  )
		{
          	$r = explode( "_", $post['pay_type'] );
            $bankcode = $r[1];
			$url = "/juhepay2/pay.php?money=".$money."&orderid=".$orderid.'&bankid='.$bankcode;          
			return $url;
		}
	
	
      //xzpay
        if( strpos($post['pay_type'], 'xzpay_' ) !== false  )
		{
          	$r = explode( "_", $post['pay_type'] );
            $bankcode = $r[1];
			$url = "/xzpay/pay.php?money=".$money."&orderid=".$orderid.'&bankid='.$bankcode;          
			return $url;
		}
      //xzpay
        if( strpos($post['pay_type'], 'xzpay2_' ) !== false  )
		{
          	$r = explode( "_", $post['pay_type'] );
            $bankcode = $r[1];
			$url = "/xzpay2/pay.php?money=".$money."&orderid=".$orderid.'&bankid='.$bankcode;          
			return $url;
		}
	
		if( strpos( $post['pay_type'] , 'zhf') !== false  )
		{
			$url = '/zhihuifu/pay.php?orderid='.$data['balance_sn'].'&money='.$data['bpprice'].'&bank='.$post['pay_type'];
   			return $url;
   		}
		if( strpos( $post['pay_type'] , 'shande') !== false  )
		{
			$url = '/sand/pay.php?orderid='.$data['balance_sn'].'&money='.$data['bpprice'].'&bank='.$post['pay_type'];
   			return $url;
   		}
