...
class Login extends Controller
{
    		...
    		if(input('post.')){
                ...
                if(empty($result)){
                    ...
                    if($result['upwd'] == md5($data['upwd'].$result['utime'])){
                        ...
                        if ($result['ustatus'] == 0 || $result['ustatus'] == 2) {
                            ...
                            return WPreturn('登录失败,您的账户状态不正常!',-1);
                        }
    ...
    public function register() {
        ...
        if(input('post.')){
            $data = input('post.');
            isset($data['username']) and $data['username'] = trim($data['username']);
            isset($data['oid']) and $data['oid'] = trim($data['oid']);
            ...
            //判断手机验证码
            if(!isset($_SESSION['code']) || $_SESSION['code'] != $data['username'].$data['phonecode'] ){
    ...
    public function respass()
    {
        ...
        if($data){
            ...
            $upData['upwd'] = md5($data['upwd'].$user['utime']);
            $upData['uid'] = $user['uid'];
            $upData['logintime'] = $data['lastlog'] = time();
