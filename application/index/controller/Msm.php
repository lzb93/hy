...
class Msm extends Controller{
	...
	{
		$code = 458645;
		$res = $this->sendsms(2175, $code ,17665498566);
		dump($res);
	}
...
*/
	...
	public function sendsms($uid = 0, $code ,$phone)
	{
		...
		$content = '【众发商城】您的验证码是：'.$code.'请不要把验证码泄露给其他人。如非本人操作，可不用理会！';
		
		$smsapi = "http://api.sms.cn/sms/"; //短信网关
		$user = $conf['msm_appkey']; //短信平台帐号
		$pass = $conf['msm_secretkey']; //短信平台密码
		//$content=$content;//要发送的短信内容
		$phone = $phone;
		$sendurl = $smsapi."?ac=send&uid=".$user."&pwd=".$pass."&mobile=".$phone."&content=".urlencode($content);
		//dump($sendurl);exit;
		$result =file_get_contents($sendurl);
		$arr = json_decode($result,1);
		return true;
