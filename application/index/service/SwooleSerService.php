...
class SwooleSerService {
    ...
    function __construct($settings) {
        ...
        if (!file_exists($this->settings['log_file'])) {
            $this->mkdirs(dirname($this->settings['log_file']));
        }
    }
    ...
    public function mkdirs($filename, $mode = 0777) {
        $position = strrpos($filename, '/');
        $path = substr($filename, 0, $position);
        if (!file_exists($path)) {
            mkdir($filename, $mode, true); //创建目录 
            chmod($filename, $mode);
        }
        return true;
    }
    ...
    public function serviceStart() {
        ...
        //启动
        $server = new SwooleService($this->settings);
        $server->run();
    }
