...
function myoids($uid,$ids=[])
{
	...
	$list = Db::name('userinfo')->field('uid')->where($map)->select();
...
function myuids($uid) {
    ...
    $user = Db::name('userinfo')->field('uid')->where($map)->select();
...
function myupoid($uid) {
    ...
    $user = Db::name('userinfo')->field('uid,oid,rebate,usermoney,feerebate,minprice')->where($map)->find();
...
function mytime_oids($uid) {
    ...
    $list = Db::name('userinfo')->field('uid,oid,username,utel,nickname,usermoney')->where($map)->select();
    $uids = array();
    foreach ($list as $key => $v) {
	                  ...
	                  <div class="vo_son" style="margin-left: ' . $margin_left . 'px;"><p>|——' . $type . '级代理</p></div>
	                    <div class="div_my_son">
	                      <ul class="my_sons">
	                        ...
	                        <li>手机：' . $vo['utel'] . ' <a href="/dladminameiqj/user/userlist.html?uid=' . $vo['uid'] . '"><button class="btn btn-primary btn-xs">详情</button></a></li>
	                      </ul>
...
function ChickIsOpen($pid) {
    ...
    $isopen = 0;
    $pro = Db::name('productinfo')->where(array('pid'=>$pid))->find();
    ...
    if ($pro['isopen']) {
        $opentime = Db::name('opentime')->where('pid='.$pid)->find();
...
function cash_oid($uid) {
    ...
    $user = Db::name('userinfo')->where('uid', $uid)->field('uid,usermoney,minprice')->find();
    if (!$user['minprice'])
        $user['minprice'] = 0;
    ...
    return '<td> <a title="点击查看" href="/dladminameiqj/user/userlist.html?uid=' . $uid . '"> ' . $uid . ' </a> </td><td ' . $class . '>' . $minprice . '</td>';
}
...
function check_user($field, $value) {
    ...
    $isset = Db::name('userinfo')->where($field, $value)->value('uid');
    if ($isset) {
        return true;
    } else {
        return false;
...
function getuser($uid, $field) {
    ...
    $value = Db::name('userinfo')->where('uid', $uid)->value($field);
    return $value;
}

function ordernum($uid) {

	$num = Db::name('order')->where('uid',$uid)->count();
	if(!$num) $num = 0;
	return $num;
