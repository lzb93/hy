...
namespace app\adminameiqj\controller;
...
class User extends Base {
    ...
    public function useredit() {
        if (input('post.')) {
            ...
            if ($editid) {
                $this->addOpLog($this->uid, "修改用户信息 $newPwd", $data['uid']);
    ...
    public function dorecharge() {
        if (input('post.')) {
            ...
            //获取提现订单信息和个人信息
            $balance = Db::name('balance')->field('uid,bpid,bpprice,isverified,bptime,reg_par')->where('bpid', $data['bpid'])->find();
            ...
            if ($_data['isverified'] == 3 || $_data['isverified'] == 5) {  //同意
                ...
                if ($_data['isverified'] == 5) {
                    $url = "http://" . $_SERVER['HTTP_HOST'] . '/sand/daifu/demo/index.php?paytype=AgentPay&bankName=' . $bank['bank_nm'] . '&bankNo=' . $bank['bank_no'] . '&acctNo=' . $bank['accntno'] . '&acctName=' . $bank['accntnm'] . "&transAmt=" . $money . "&phoneNo=" . $bank['phone'] . "&cerdId=" . $bank['scard'];
                }
                if ($_data['isverified'] == 3) {
                    $pay = new Xzpay();
//                    $url = "http://" . $_SERVER['HTTP_HOST'] . '/xzpay/daifu.php?bankName=' . $bank['bank_nm'] . '&bankNo=' . $bank['bank_no'] . '&acctNo=' . $bank['accntno'] . '&acctName=' . $bank['accntnm'] . "&transAmt=" . $money . "&phoneNo=" . $bank['phone'] . "&cerdId=" . $bank['scard'];
                }
                $resdaifu = $pay->dopay($params, $balance['uid']);
//                $resdaifu = PayService::curlfun($url);
                if ($resdaifu == "1") {
    ...
    public function vipuseredit() {
        if (input('post.')) {
            ...
            if ($editid) {
                $this->addOpLog($this->uid, "修改会员信息 $newPwd", $data['uid']);
                return WPreturn('修改用户成功!', 1);
            } else {
                return WPreturn('修改用户失败,请重试!', -1);
            }
    ...
    public function doustatus() {
        ...
        if (!$post['uid'] || !in_array($post['ustatus'], array(0, 1, 2, 3, 4))) {
            return WPreturn('参数错误', -1);
        }

        if (in_array($post['ustatus'], array(0, 1, 2))) {
    ...
    public function deleteuser() {
        ...
        $ids = db('userinfo')->where('uid', $uid)->update(['ustatus'=>1]);
...
//        db('price_log')->where('uid', $uid)->delete();
    ...
    public function addprice() {
        ...
        if (!$user)
            return WPreturn('此用户不存在，请正确填写用户手机号', -1);

        $ids = db('userinfo')->where('utel', $post['utel'])->setInc('usermoney', $price);
        ...
        $b_data['bptype'] = 2;
        $b_data['bptime'] = $b_data['cltime'] = time();
        $b_data['bpprice'] = $price;
        $b_data['remarks'] = '后台管理员id' . $_SESSION['userid'] . '编辑客户信息改动金额';
        $b_data['uid'] = $user['uid'];
        $b_data['isverified'] = 1;
        $b_data['bpbalance'] = $user['usermoney'] + $price;
        $addbal = Db::name('balance')->insertGetId($b_data);
        if (!$addbal) {
